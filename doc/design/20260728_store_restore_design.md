...
- **S3 互換サーバのフレキシブルチェックサム対応**: RustFS が `x-amz-checksum-sha256` を受け付け、保管し、`GetObject` で返すかは実機確認が要る。受け付けない実装では `PutObject` 自体が失敗しうる
  - 緩和策: 6.7.1 のベストエフォート方針（非対応起因の失敗を検出したらチェックサム無しで 1 回だけ再試行）と `CAFCE_S3_CHECKSUM` による明示制御を設計に織り込んだ。残る不確実性は「非対応時にどのエラーコードが返るか」がサーバ実装依存である点で、これは統合テストと実機確認で詰める
  - 判定を誤って不要なフォールバックをしても、metadata ハッシュ検証は残るため安全側に倒れる。逆に、検証が黙って外れることを許容できない運用は `CAFCE_S3_CHECKSUM=required` を使う
