...
1. **ファイルパターン解決**:
   ...
   - `base_path`からの相対パスのみ対応。絶対パスのパターンはカレントディレクトリより
     外側の探索につながるため明示的にエラーとする（§8参照）
   - 存在しないファイルは無視（GitLab CI互換）

2. **ハッシュ計算**:
   - 各ファイルの内容をSHA-256でハッシュ化する（ファイルパス文字列はハッシュ入力に
     含めない。含めるとチェックアウト先や`base_path`の違いだけで同一内容でも
     異なるキーになり、GitLab CI互換の目標を満たせなくなるため）
   - ファイルパスで昇順ソートして結合順序を固定した上で、各ファイルの内容ハッシュを
     連結し、その全体をさらにSHA-256でハッシュ化して最終ハッシュとする
   - プレフィックスが指定されている場合は結合
   - パターンが1件もマッチしなかった場合は、固定値ではなくエラーとして報告する
     （設定ミスと本来ファイルが存在しないケースを区別できなくなるため）
...
1. **ファイルパターン解決** ✅
   ...
   - `base_path`からの相対パスのみ対応。絶対パスのパターンは`CacheKeyError::AbsolutePathNotAllowed`で明示的に拒否
   - 存在しないファイルの無視（GitLab CI互換）
   - セキュリティ対策（base_pathより外側のファイルアクセス防止、`base_path`自体のglobメタ文字エスケープ）

2. **ハッシュ計算** ✅
   - 各ファイルの内容をSHA-256でハッシュ化（ファイルパス文字列はハッシュ入力に含めない）
   - ファイルパスでソートして結合順序を固定した上で、各ファイルの内容ハッシュを連結し最終ハッシュを計算
   - プレフィックス対応（`prefix-{hash}`形式）
   - マッチ0件の場合は固定ハッシュではなく`CacheKeyError::NoFilesMatched`で報告
...
#### 単体テスト: 28/28 成功 ✅
- **FileMatcher**: 10テスト（基本機能、ワイルドカード、制限、絶対パス拒否、globメタ文字エスケープ、エラーケース）
- **HashCalculator**: 10テスト（単一ファイル、複数ファイル、順序保証、既知値による回帰、エラーケース）
- **CacheKeyGenerator**: 8テスト（基本機能、プレフィックス、複数パターン、変更検知、マッチ0件エラー）
...
### 2026/07/20
- コードレビュー指摘への対応
  - `HashCalculator::calculate_files_hash`がファイルの絶対パスをハッシュに
    混入させ、GitLab CI互換性の目標を満たせていなかった問題を修正
  - `FileMatcher`が絶対パスパターンをbase_path外として無音で除外していた
    問題を修正し、`CacheKeyError::AbsolutePathNotAllowed`で明示的に拒否
  - `base_path`にglobメタ文字が含まれる場合の誤動作を修正
  - パターンのマッチが0件の場合に固定ハッシュを返していた問題を修正し、
    `CacheKeyError::NoFilesMatched`で報告するよう変更
  - 未使用のエラーバリアントを削除
  - 既知のハッシュ値による回帰テストを追加
  - 単体テスト28/28、統合テスト5/5成功を維持
