...
/// 実際のAWS S3に対する疎通確認テスト（静的クレデンシャル・AssumeRole・Profileの3パターン）。
///
/// このモジュールのテストは通常の`cargo test`実行ではスキップされる
...
/// - Profileテスト（`aws_profile_smoke_test`）用に、上記IAMユーザーの静的クレデンシャルを
///   使ったプロファイルを、テスト実行環境の`~/.aws/credentials`・`~/.aws/config`に
///   事前に設定しておくこと（cafce自体はこのプロファイルの中身を読まず、
///   プロファイル名をSDKに渡してcredential解決を委ねるだけである）
///
/// 実行に必要な環境変数:
...
/// - `CAFCE_AWS_PROFILE` - Profileテスト（`aws_profile_smoke_test`）でのみ必要。
///   `~/.aws/config`・`~/.aws/credentials`に設定済みのプロファイル名
///   （例: "cafce-test-profile"）
///
...
/// export CAFCE_AWS_PROFILE=cafce-test-profile          # Profileテストのみ必要
...
mod aws_integration_tests {
    ...
    /// 実際のAWS S3に対して、AWS Profile（`~/.aws/config` / `~/.aws/credentials`）
    /// 経由での疎通確認を行う。
    ///
    /// cafce側ではaccess_key/secret_key/session_token/role_arn/role_session_nameを
    /// 一切設定せず、profileにプロファイル名のみを渡す。これによりbuild_s3_client()の
    /// 静的クレデンシャル・AssumeRoleロジックを完全にバイパスし、SDKのconfig loaderに
    /// クレデンシャル解決を委ねる分岐（`.profile_name(profile)`）を確認する。
    ///
    /// put_object -> list_objects_v2 -> get_object -> delete_object の一連の流れを、
    /// 既存のテスト専用バケットに対して行う（create_bucket/delete_bucketは呼ばない。
    /// IAMポリシーにバケット作成・削除権限が無いため）。
    ///
    /// 実行方法:
    ///   export CAFCE_AWS_REGION=... CAFCE_TEST_BUCKET=... CAFCE_AWS_PROFILE=cafce-test-profile
    ///   # ~/.aws/config・~/.aws/credentialsにcafce-test-profileを事前に設定しておくこと
    ///   cargo test aws_profile_smoke_test -- --ignored --nocapture
    #[tokio::test]
    #[ignore]
    async fn aws_profile_smoke_test() {
        let region = std::env::var("CAFCE_AWS_REGION").unwrap_or_else(|_| {
            panic!(
                "環境変数CAFCE_AWS_REGIONが設定されていません。\
                 テスト対象バケットのリージョン（例: ap-northeast-1）を指定してください。"
        ...
        let profile = std::env::var("CAFCE_AWS_PROFILE").unwrap_or_else(|_| {
            panic!(
                "環境変数CAFCE_AWS_PROFILEが設定されていません。\
                 ~/.aws/config・~/.aws/credentialsに事前設定したプロファイル名\
                 （例: cafce-test-profile）を指定してください。"
            )
        });
        ...
        let client = build_s3_client(&env)
            .await
            .expect("failed to build S3 client for AWS S3 (Profile)");

        let key = unique_object_key("cafce-profile-test");
        let content: &[u8] = b"hello from cafce (Profile)";
